...
# Each entry is (url, listing_type)
PAGES = [
...
def parse_date(raw: str):
    # Forum dates look like: "05-13-2026, 06:26 PM"
    try:
...
def scrape_page(url: str) -> list:
    ...
    for cell in soup.find_all("td", class_="cell-topic"):
        ...
        # Prepend the forum prefix (e.g. "For Sale:", "WTB:") if present
        label_tag = wrapper.find("a", class_="js-topic-prefix")
        if label_tag:
            label = label_tag.get_text(strip=True).rstrip(":")
            title = f"[{label}] {title}"

        # Date is inside div.topic-info > span.date
        posted_at = None
